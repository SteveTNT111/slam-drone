...
class MockArrivalTimeoutTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_arrival_timeout_requests_auto_land(self):
        ...
        final_state = None
        saw_failsafe_land_reason = False
        while time.time() < deadline and not rospy.is_shutdown():
            with self.lock:
                final_state = (self.last_task_state or {}).get("state")
                reason = (self.last_task_state or {}).get("reason", "")
                if "did not reach stable takeoff height" in reason:
                    saw_failsafe_land_reason = True
            if final_state == "COMPLETE":
                break
            time.sleep(0.02)
        self.assertEqual(final_state, "COMPLETE")
        self.assertTrue(saw_failsafe_land_reason)

        time.sleep(0.15)
        with self.lock:
            count_after_land = self.setpoint_count
        time.sleep(0.35)
        with self.lock:
            self.assertEqual(self.setpoint_count, count_after_land)
            self.assertIn("AUTO.LAND", self.mode_requests)


if __name__ == "__main__":
    rospy.init_node("mock_arrival_timeout_test")
    rostest.rosrun("px4_basic_control", "mock_arrival_timeout_test", MockArrivalTimeoutTest)
