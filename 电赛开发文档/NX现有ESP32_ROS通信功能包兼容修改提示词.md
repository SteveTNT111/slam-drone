...
8. 必须明确完成两个命令到飞机现有任务入口的映射：
   - `Drone_Task1Off` -> 调用或发布飞机现有“任务1启动”接口；该任务后续的解锁、OFFBOARD、起飞和任务流程由现有飞行任务节点执行；
   - `Drone_Task2Off` -> 调用或发布飞机现有“任务2启动”接口；该任务后续的解锁、OFFBOARD、起飞和任务流程由现有飞行任务节点执行。

9. 必须先审计现有飞行任务节点真实使用的话题、服务、action 或状态机入口，不得猜测接口名称。如果当前只有 ESP32 通信包、尚无任务1/任务2入口，应明确报告缺失接口，不允许在串口读线程中直接拼接 MAVROS 起飞代码。

10. 验收时必须证明：每收到一次合法 `Drone_Task1Off` 或 `Drone_Task2Off`，只产生一次对应的 ROS 高层任务启动事件；重复帧只能重复 ACK，不能重复起飞。
