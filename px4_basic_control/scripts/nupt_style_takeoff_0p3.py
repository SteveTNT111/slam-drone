#!/usr/bin/env python3
"""NUpt-style PX4 OFFBOARD takeoff, hover, setpoint descent, and ground disarm."""
...
class OneKeyTakeoffHoverLandNode:
    ...
    def __init__(self):
        ...
        self.hover_duration = float(rospy.get_param("~hover_duration", 5.0))
        ...
        self.ground_height_tolerance = float(rospy.get_param("~ground_height_tolerance", 0.03))
        ...
        self.request_interval = float(rospy.get_param("~service_request_interval", 1.0))
        ...
        self.state_pub = rospy.Publisher(
            "/uav/one_key_takeoff_hover_land/state", String, queue_size=10, latch=True
        )
        self.target_pub = rospy.Publisher(
            "/uav/one_key_takeoff_hover_land/active_target", PoseStamped, queue_size=10
        )
        ...
        rospy.Subscriber(
            "/mavros/local_position/pose", PoseStamped, self._local_pose_callback, queue_size=20
        )
        rospy.Subscriber(
        ...
        rospy.logwarn(
            "NUpt-style protected test: no SLAM/vision attitude-alignment gate; "
            "press Enter to capture the current PX4 local pose"
        )
    ...
    def _input_worker(self):
        ...
        try:
            input(">>> 飞机保持未解锁 POSCTL；确认人员退开后按 Enter 起飞，Ctrl-C 取消：")
        ...
        with self.lock:
            ...
            self.target.pose.position.x = pose.position.x
            self.target.pose.position.y = pose.position.y
            self.target.pose.position.z = pose.position.z
            self._set_target_yaw(yaw)
    ...
    def _request_ground_forced_disarm_async(self):
        """Force-disarm only after the state machine's redundant ground gate."""
        if self.arm_request_running:
            return
        self.arm_request_running = True
        ...
        with self.lock:
            ...
            if self.offboard_seen and state is not None and state.armed:
                if state.mode != self.offboard_mode:
                    self._transition(self.PILOT_TAKEOVER, "pilot changed mode to %s" % state.mode)
                    rospy.logwarn("pilot takeover confirmed; OFFBOARD will not be requested again")
            ...
            if self.state_name == self.PRESTREAM:
                ...
                if state is not None and state.mode != self.offboard_mode:
                    ...
                    self._transition(self.TAKEOFF, "PX4 confirmed armed; sending +%.2f m" % self.takeoff_height)
                ...
                if self.local_pose is not None:
                    ...
                    if error <= self.height_tolerance:
                        if self.arrival_confirmed_since is None:
                            ...
                            self._transition(
                                self.HOVER,
                                "PX4 local height held within tolerance for %.1f s"
                ...
                if now - self.hover_started >= self.hover_duration:
                    ...
                    self._transition(
                        self.DESCEND,
                        "5 second hover complete; descending in OFFBOARD while holding yaw",
                    )
                ...
                local_z = self.local_pose.pose.position.z if self.local_pose is not None else float("nan")
                rospy.loginfo_throttle(
                    1.0,
                ...
                if state is not None and not state.armed:
                    ...
                    self.ground_confirmed_since = None
                    self._transition(self.DESCEND, "ground confirmation was lost; disarm cancelled")
                elif now - self.last_arm_request >= self.request_interval:
                    self.last_arm_request = now
...
def main():
    rospy.init_node("one_key_takeoff_hover_land")
