...
function draw() {
  ...
  const pad = Math.max(24 * state.canvasScale, Math.min(w, h) * 0.035);
...
function updatePanels() {
  ...
  if (state.drone) {
    ...
    els.batteryV.textContent = fmt(state.drone.battery, state.drone.batteryUnit || "V", state.drone.batteryUnit === "%" ? 0 : 2);
...
function applyTelemetry(msg) {
  ...
  if (msg.kind === "car") {
    markNode("car", "收到小车遥测");
    // 小车发实际 X, Y 坐标(cm), 网页直接显示
    // 模拟器也发 x_cm/y_cm(由 carPoseFromProgress 生成), 两者统一走 x_cm/y_cm
  ...
  if (msg.kind === "drone") {
    ...
    state.drone = {
      ...
      battery: msg.battery_v ?? msg.battery_pct ?? 0,
      batteryUnit: msg.battery_v !== undefined ? "V" : (msg.battery_pct !== undefined ? "%" : "V"),
      phase: msg.phase ?? 0,
...
async function readSerialLoop() {
  /* 直接读原始字节(不走 TextDecoderStream 管道), 遇到 Framing error 等
   * 瞬态错误时释放 reader、短暂等待后重新获取, 不中断读取循环。 */
  while (serialKeepReading) {
    try {
      ...
      if (!serialKeepReading) break;
      addSerialLog(`读取中断：${error.message}，自动重连…`, "错误");
    } finally {
