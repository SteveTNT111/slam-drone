...
> 下面的目录使用 Obsidian 页内跳转格式，点击即可跳到对应章节。

### 完整目录

- [[#1. 项目简介|1. 项目简介]]
  - [[#1.1 当前数据链路|1.1 当前数据链路]]
  - [[#1.2 这份文档和浙大 README 的关系|1.2 这份文档和浙大 README 的关系]]
- [[#2. 项目结构与目录来源|2. 项目结构与目录来源]]
  - [[#2.1 浙大原始路径和当前路径对照|2.1 浙大原始路径和当前路径对照]]
  - [[#2.2 浙大原始实飞文件和当前 FAST-LIO2 版文件对照|2.2 浙大原始实飞文件和当前 FAST-LIO2 版文件对照]]
- [[#3. 文件夹名称、缩写和功能速查|3. 文件夹名称、缩写和功能速查]]
  - [[#3.1 当前 `catkin_ws/src` 里的包|3.1 当前 catkin_ws/src 里的包]]
  - [[#3.2 名称缩写怎么理解|3.2 名称缩写怎么理解]]
  - [[#3.3 EGO 相关包之间的关系|3.3 EGO 相关包之间的关系]]
- [[#4. px4ctrl 是什么，怎么控制飞机|4. px4ctrl 是什么，怎么控制飞机]]
  - [[#4.1 px4ctrl 使用什么模式控制 PX4|4.1 px4ctrl 使用什么模式控制 PX4]]
  - [[#4.2 四元数加油门和遥控器四通道有什么区别|4.2 四元数加油门和遥控器四通道有什么区别]]
  - [[#4.3 px4ctrl 订阅和发布的话题|4.3 px4ctrl 订阅和发布的话题]]
  - [[#4.4 px4ctrl 的状态机|4.4 px4ctrl 的状态机]]
  - [[#4.5 起飞、悬停、G 点目标后的真实模式|4.5 起飞、悬停、G 点目标后的真实模式]]
  - [[#4.6 遥控器通道和紧急手动介入|4.6 遥控器通道和紧急手动介入]]
  - [[#4.7 px4ctrl 配置文件在哪里|4.7 px4ctrl 配置文件在哪里]]
- [[#5. 悬停油门自动学习和标定脚本|5. 悬停油门自动学习和标定脚本]]
  - [[#5.1 自动油门估计的逻辑|5.1 自动油门估计的逻辑]]
  - [[#5.2 我需不需要先做油门标定|5.2 我需不需要先做油门标定]]
  - [[#5.3 先用定点模式收集悬停数据|5.3 先用定点模式收集悬停数据]]
  - [[#5.4 自动起飞和降落脚本|5.4 自动起飞和降落脚本]]
  - [[#5.5 thrust_calibrate.py 怎么用|5.5 thrust_calibrate.py 怎么用]]
- [[#6. EGO-Planner 使用什么输入|6. EGO-Planner 使用什么输入]]
  - [[#6.1 两个配置文件各负责什么|6.1 两个配置文件各负责什么]]
- [[#7. FAST-LIO2 和 VINS/Realsense 怎么切换|7. FAST-LIO2 和 VINS/Realsense 怎么切换]]
  - [[#7.1 当前雷达方案|7.1 当前雷达方案]]
  - [[#7.2 原始视觉方案|7.2 原始视觉方案]]
  - [[#7.3 按浙大教程，实飞前首先要改什么|7.3 按浙大教程，实飞前首先要改什么]]
  - [[#7.4 换成 FAST-LIO2 后要改哪些文件|7.4 换成 FAST-LIO2 后要改哪些文件]]
  - [[#7.5 launch 和 XML 文件具体改哪里|7.5 launch 和 XML 文件具体改哪里]]
  - [[#7.6 Ceres 和 glog 还需要吗|7.6 Ceres 和 glog 还需要吗]]
- [[#8. EGO-Planner 输出什么给 px4ctrl|8. EGO-Planner 输出什么给 px4ctrl]]
- [[#9. RViz 按 G 设置目标点到底发了什么|9. RViz 按 G 设置目标点到底发了什么]]
  - [[#9.1 二维目标点代码解析|9.1 二维目标点代码解析]]
- [[#10. 如何给 EGO-Planner 自定义航点|10. 如何给 EGO-Planner 自定义航点]]
  - [[#10.1 固定航点：用 flight_type=2|10.1 固定航点：用 flight_type=2]]
  - [[#10.2 单个动态目标：发布 /move_base_simple/goal|10.2 单个动态目标：发布 /move_base_simple/goal]]
  - [[#10.3 比赛任务：写一个航点管理节点|10.3 比赛任务：写一个航点管理节点]]
- [[#11. EGO-Planner 关键参数在哪里改|11. EGO-Planner 关键参数在哪里改]]
- [[#12. 使用方法：编译、同步和启动|12. 使用方法：编译、同步和启动]]
  - [[#12.1 原始 sh 脚本和当前脚本用途对照|12.1 原始 sh 脚本和当前脚本用途对照]]
  - [[#12.2 第一次实飞前如何手持测试 EGO-Planner|12.2 第一次实飞前如何手持测试 EGO-Planner]]
  - [[#12.3 第一次带桨实飞前的最低要求|12.3 第一次带桨实飞前的最低要求]]
- [[#13. 起飞前必须检查的话题|13. 起飞前必须检查的话题]]
- [[#14. 目录问题统一解释|14. 目录问题统一解释]]
- [[#15. 给零基础用户的最小操作顺序|15. 给零基础用户的最小操作顺序]]
- [[#16. 当前还不建议交给用户乱改的地方|16. 当前还不建议交给用户乱改的地方]]
## 1. 项目简介
