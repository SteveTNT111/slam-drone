...
当前实机无线协议以小车的 68 字节 `espnow_msg_t` 为准：

- 小车遥测由 `kind="Car"` 表示；
- 无人机遥测由 `kind="Drone"` 表示；
- 无人机电量字段是百分比，因此网页同时兼容 `battery_pct` 与原有 `battery_v`；
- D_UNIT 与队友 ESP32-S3 地面端输出相同字段的单行 JSON；
- 网页仍然只负责接收和显示，任务按钮尚未连接到真实无线控制链路。

真实设备输出示例：

```json
{"kind":"drone","x_cm":50,"y_cm":120,"height_cm":100,"yaw_deg":90.0,"horizontal_speed_cm_s":20.5,"vertical_speed_cm_s":0,"target_error_cm":0,"battery_pct":86,"phase":3,"seq":8,"source":2}
```
